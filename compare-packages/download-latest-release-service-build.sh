...
function getBuiltExecutionId {
	curl -s $api/builds/1_20140731_international_release_build/executions 2>/dev/null | grep -A2 "\"id\" : \"$today" | grep -B2 '"status" : "BUILT"' | head -n1 | awk -F \" '{print $4}'
}
...
if [ "$builtExecutionId" != "" ]; then
	...
	releaseFile=$(curl -s ${api}/builds/1_20140731_international_release_build/executions/${builtExecutionId}/packages/snomed_release_package/outputfiles | grep '"url"' | grep '.zip"' | awk -F \" '{ print $4 }')
