...
public class Rf2FileExportRunner {
			...
			for (FolderType subFolderTypeLevel1 : folderTypes) {
				if ((isDeltaFolderExistInManifest && subFolderTypeLevel1.getName().equalsIgnoreCase(RF2Constants.DELTA))
					...
					for (FolderType subFolderTypeLevel2 : subFolderTypeLevel1.getFolder()) {
						files = ManifestFileListingHelper.listAllFiles(subFolderTypeLevel2, SRC_ADDITIONAL_PACKAGE);
					}
	...
	private void generateAdditionalReleaseFile(final String additionalDeltaDataFile, final Map<String, List <Integer>> customRefsetCompositeKeys) throws BusinessServiceException {
		...
		if (CollectionUtils.isEmpty(configuration.getAdditionalPackages())) {
			throw new BusinessServiceException("The additional previous published packages must not be empty");
		}

		List<BuildAdditionalPackage> buildAdditionalPackages = configuration.getAdditionalPackages();
		...
		try {
			...
			for (BuildAdditionalPackage aPackage : buildAdditionalPackages) {
				InputStream additionalDeltaInputStream = getEquivalentAdditionalPackageInputStream(aPackage.getReleaseCenterKey(), aPackage.getAdditionalPackageName(), additionalDeltaDataFile);
			...
			for (BuildAdditionalPackage aPackage : buildAdditionalPackages) {
				InputStream additionalFullInputStream = getEquivalentAdditionalPackageInputStream(aPackage.getReleaseCenterKey(), aPackage.getAdditionalPackageName(), currentFullFileName);
	...
	private InputStream getEquivalentAdditionalPackageInputStream(String releaseCenterKey, String additionalPreviousPublishedPackage, String currentAdditionalFileName) throws IOException {
		return buildDao.getPublishedFileArchiveEntry(releaseCenterKey, getEquivalentAdditionalFile(additionalPreviousPublishedPackage, currentAdditionalFileName), additionalPreviousPublishedPackage);
	}
