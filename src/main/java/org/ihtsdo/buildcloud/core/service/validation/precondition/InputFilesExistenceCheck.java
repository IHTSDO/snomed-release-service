...
import static org.ihtsdo.buildcloud.core.service.InputFileServiceImpl.SRC_ADDITIONAL_PACKAGE;
...
public class InputFilesExistenceCheck extends PreconditionCheck {
	...
	private Set<String> resolveExpectedFilesByManifest(final boolean isBeta, final boolean isJustPackaging, final ListingType listingType) {
		...
		List<String> filesFromAdditionalPackage = ManifestFileListingHelper.listAllFiles(listingType, SRC_ADDITIONAL_PACKAGE);
		...
		for ( String fileName : ManifestFileListingHelper.listAllFiles(listingType)) {
			...
			final String[] splits = fileName.split(RF2Constants.FILE_NAME_SEPARATOR);
			String fileNamePrefix = splits[0];
			if (!isFirstReadmeFound && RF2Constants.README_FILENAME_PREFIX.equals(fileNamePrefix)) {
				//ignore the first one as it will be auto generated by release service.
				isFirstReadmeFound = true;
				continue;
			}
			if (RF2Constants.RELEASE_INFORMATION_FILENAME_PREFIX.equals(fileNamePrefix)) {
			...
			if (!isJustPackaging) {
				...
				if (RF2Constants.DER2.equals(fileNamePrefix) || RF2Constants.SCT2.equals(fileNamePrefix)) {
					...
					if (token3.contains(RF2Constants.FULL)) {
						fileName = fileName.replace(RF2Constants.FULL, RF2Constants.DELTA);
					} else if (token3.contains(RF2Constants.SNAPSHOT)) {
						fileName = fileName.replace(RF2Constants.SNAPSHOT, RF2Constants.DELTA);
					}
				} 
			}
	...
	private boolean isFromAdditionalRelease(List<String> filesFromAdditionalPackage, String fileName) {
		return !filesFromAdditionalPackage.isEmpty() && (filesFromAdditionalPackage.contains(fileName)
				|| (filesFromAdditionalPackage.get(0).contains(RF2Constants.DELTA) && (filesFromAdditionalPackage.contains(fileName.replace(RF2Constants.SNAPSHOT, RF2Constants.DELTA)) || filesFromAdditionalPackage.contains(fileName.replace(RF2Constants.FULL, RF2Constants.DELTA))))
				|| (filesFromAdditionalPackage.get(0).contains(RF2Constants.SNAPSHOT) && filesFromAdditionalPackage.contains(fileName.replace(RF2Constants.FULL, RF2Constants.SNAPSHOT))));
	}
}
