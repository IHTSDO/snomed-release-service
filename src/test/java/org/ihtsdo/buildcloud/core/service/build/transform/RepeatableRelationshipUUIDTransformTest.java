...
public class RepeatableRelationshipUUIDTransformTest {
	...
	public void testTransformSameLineAlwaysGivesSameUUID() throws Exception {
		...
		assertEquals(RF2Constants.NULL_STRING, valuesA1[0]);
		transform.transformLine(valuesA1);
		assertEquals("c5c20346-62d2-5fb1-8df4-82fbdbb4cc49", valuesA1[0]);

		final String[] valuesA2 = lineA.split("\\t");
		assertEquals(RF2Constants.NULL_STRING, valuesA2[0]);
		transform.transformLine(valuesA2);
		assertEquals("c5c20346-62d2-5fb1-8df4-82fbdbb4cc49", valuesA2[0]);

		final String[] valuesB1 = lineB.split("\\t");
		assertEquals(RF2Constants.NULL_STRING, valuesB1[0]);
		transform.transformLine(valuesB1);
		assertEquals("81342da0-c3df-5ce2-900a-95fd6be13885", valuesB1[0]);

		final String[] valuesB2 = lineB.split("\\t");
		assertEquals(RF2Constants.NULL_STRING, valuesB2[0]);
		transform.transformLine(valuesB2);
		assertEquals("81342da0-c3df-5ce2-900a-95fd6be13885", valuesB2[0]);
	}
