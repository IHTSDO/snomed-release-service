...
public class ReleasePackageCheck extends PostconditionCheck implements NetworkRequired {
    ...
    private String validateBetaReleasePackage(Build build) throws ResourceNotFoundException{
        ...
        if (zippedFileName != null) {
            ...
            zippedFileName = zippedFileName.substring(zippedFileName.lastIndexOf("/") + 1, zippedFileName.length());
            ...
            try {
                ...
                for (File file : files) {
                    if (!file.isDirectory()) {
                        if (!file.getName().startsWith(README_FILENAME_PREFIX) &&
                                !file.getName().endsWith(README_FILENAME_EXTENSION) &&
                                !file.getName().startsWith(RELEASE_INFORMATION_FILENAME_PREFIX) &&
                                !file.getName().endsWith(RELEASE_INFORMATION_FILENAME_EXTENSION) &&
