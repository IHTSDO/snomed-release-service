...
public class ConfigurationCheck extends PreconditionCheck {
    ...
    @Override
    public final void runCheck(final Package pkg, final Execution execution) {
	...
	Date effectiveTime = pkg.getBuild().getEffectiveTime();
	if (effectiveTime == null) {
	    errorList.add(NO_EFFECTIVE_TIME);
	}
	// check the published release date is in the past
	if (pkg.getPreviousPublishedPackage() != null && effectiveTime != null) {
	    String[] tokens = pkg.getPreviousPublishedPackage().split(RF2Constants.FILE_NAME_SEPARATOR);
	    if (tokens.length > 0) {
